...
public class MutilLoginServiceImpl implements IMutilLoginService {
	private static Logger LOG = LoggerFactory.getLogger(MutilLoginServiceImpl.class);
	...
	public MutilLoginServiceImpl(IMsgHandlerFace msgHandler) {
		core = new MultiCore();
	...
	public boolean webWxInit() {
		...
		try {
			...
			LOG.info("webWxInit:syncKey:"+syncKey);
	...
	public void startReceiving() {
		...
		if (core.isAlive()) {
			try {
				LOG.info(core.getNickName()+"core开始接受消息");
	...
	private Map<String, String> syncCheck() {
		...
		LOG.info(String.format(" sync check url: %s", url));
		List<BasicNameValuePair> params = new ArrayList<BasicNameValuePair>();
		...
		LOG.info(String.format(" sync check params: %s", JSON.toJSON(params)));
		SleepUtils.sleep(7);
		try {
			...
			LOG.info(String.format(" sync check text: %s", text));
